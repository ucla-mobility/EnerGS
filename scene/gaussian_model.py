...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float, voxel_size : float = 0.1):
        self.spatial_lr_scale = spatial_lr_scale
        
        # Voxel downsampling
        points = np.asarray(pcd.points)
        colors = np.asarray(pcd.colors)
        if voxel_size > 0 and points.shape[0] > 0:
            ...
            print(f"Voxel downsampling (voxel_size={voxel_size}m): {len(pcd.points)} -> {len(points)} points")
